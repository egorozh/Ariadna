...
internal class IconsStorageViewModel : BaseViewModel
{
    ...
    public IconsStorageViewModel(IconsStorageDialog iconsStorageDialog, IInterfaceHelper interfaceHelper,
        IStorage storage, IImageHelpers imageHelpers)
    {
    ...
    public string Load()
    {
        _iconDirectory = new DirectoryInfo(_storage.IconsDirectory);
        ...
        foreach (var fileInfo in _iconDirectory.GetFiles())
        {
            if (IsIcon(fileInfo.FullName))
                Items.Add(new IconViewModel(fileInfo, _iconDirectory, _iconDirectory, _interfaceHelper));
        }
    ...
    {
        var filter = _imageHelpers.GenerateImageFilter();
        ...
        if (openFileDialog.ShowDialog() == true)
        {
            ...
            Items.Add(new IconViewModel(new FileInfo(newPath), _currentDirectory, _iconDirectory, _interfaceHelper));
        }
    }
    ...
    private void OpenFolder()
    {
        ...
        {
            if (IsIcon(fileInfo.FullName))
                Items.Add(new IconViewModel(fileInfo, _currentDirectory, _iconDirectory, _interfaceHelper));
        }
    }
